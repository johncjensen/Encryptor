...
function Placeholdem(e) {
    "use strict";
    ! function() {
        for (var e = 0, n = ["ms", "moz", "webkit", "o"], i = 0; i < n.length && !window.requestAnimationFrame; ++i) window.requestAnimationFrame = window[n[i] + "RequestAnimationFrame"], window.cancelAnimationFrame = window[n[i] + "CancelAnimationFrame"] || window[n[i] + "CancelRequestAnimationFrame"];
    ...
    return n.init = function() {
        if (n.elems = [], e.length)
            for (var i = 0; i < e.length; i++) n.elems.push(new n.PlaceholdemElem(e[i]));
